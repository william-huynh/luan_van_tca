...
const TableCongcu = ({ dsNguyenlieu = [], setRowsRemoved }) => {
  ...
  const onClickChitiet = () =>
    history.push(`/admin/nguyenlieu/chitiet/${selected[0]}`);

  const onClickCapnhat = () =>
    history.push(`/admin/nguyenlieu/chinhsua/${selected[0]}`);

  const onClickXoa = () => handleOpen();

  const handleDeleteRows = async () => {
    const { success } = await apiNguyenlieu.xoaNhieuNglieu({
      arrOfIds: selected,
    });
    if (success) {
