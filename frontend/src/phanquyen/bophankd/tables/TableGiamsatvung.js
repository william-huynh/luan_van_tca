...
const TableGiamsatvung = ({
  ...
  const handleDeleteRow = async () => {
    const { success } = await apiBophankd.bophankdXoaNhieuGSV({
      bophankdId,
      arrayOfId: selected,
    });
    if (success) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = dsGiamsatvung.map((item) => item._id);
  ...
  return (
    <>
      ...
      <DialogMaterial
        ...
        content="Xóa giám sát vùng đã chọn?"
        text1="Hủy"
        text2="Xóa"
...
export default TableGiamsatvung;
