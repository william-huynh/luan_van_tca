...
const Hanggiaoden = () => {
  ...
  const handleXacnhanHanggiaoDen = async () => {
    const { success } = await apiGiaohang.gsvXacnhan(rowId);
    if (success) {
      //setRefresh(true);
      handleClose();
  ...
  const fetchDsGiaohangDen = async () => {
    ...
    let { dsgiaohang } = await apiGiaohang.dsGiaohangDenDaily1Gsv(gsv._id);
  ...
  return (
    <>
      <Container>
        <Header title="Hàng giao đến" arrOfLinks={links} vaitro="giamsatvung" />
        <Content>
          <FilterSection>
            <TitleWrapper>
